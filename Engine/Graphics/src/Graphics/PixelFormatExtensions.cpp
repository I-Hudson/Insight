...
namespace
{
    u32 sizeOfInBits[MAX_PIXEL_FORMATS];

    u32 GetIndex(const PixelFormat format)
    {
        return (u32)format;
    }
}
...
u32 PixelFormatExtensions::SizeInBits(PixelFormat format)
{
    return sizeOfInBits[GetIndex(format)];
}

u32 PixelFormatExtensions::AlphaSizeInBits(const PixelFormat format)
{
    switch (format)
    {
...
u32 PixelFormatExtensions::ComputeScanlineCount(const PixelFormat format, int height)
{
    switch (format)
    {
    case PixelFormat::BC1_Typeless:
...
u32 PixelFormatExtensions::ComputeComponentsCount(const PixelFormat format)
{
    switch (format)
    {
    case PixelFormat::R32G32B32A32_Typeless:
