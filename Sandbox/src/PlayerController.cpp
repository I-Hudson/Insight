...
void PlayerController::OnUpdate(const float& deltaTime)
{
	...
	{
		GetEntity().GetComponent<TransformComponent>().SetPosition(GetEntity().GetComponent<TransformComponent>().GetPostion() + glm::vec3(0.0f, 0.0f, 5.0f * Time::GetDeltaTime()));
	}
	...
	{
		GetEntity().GetComponent<TransformComponent>().SetPosition(GetEntity().GetComponent<TransformComponent>().GetPostion() + glm::vec3(5.0f * Time::GetDeltaTime(), 0.0f, 0.0f));
	}
