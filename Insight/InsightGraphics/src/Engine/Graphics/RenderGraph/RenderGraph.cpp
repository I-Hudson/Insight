...
namespace Insight::Graphics
{
	...
	void RenderGraph::Execute(RenderList* renderList)
	{
...

		{
			...
			{
				m_gpuResultFunc(GPUResults::Error_Out_Of_Data);
				return;
			}
		}
	...
	void RenderGraph::BuildPhysical()
	{
		...
		for (auto& resource : m_resources)
		{
			if (resource.GetPhysicalIndex() == RenderGraphResource::Unused)
			{
				...
				{
					resource.TextureInfo.m_info.Width = m_renderGraphRenderSize.x;
					resource.TextureInfo.m_info.Height = m_renderGraphRenderSize.y;
				}
	...
	void RenderGraph::SwapchainRebuild(u32 width, u32 height)
	{
		...
		m_swapchainPresentPass.SetWindowRect(Maths::Rect(0, 0, (float)width, (float)height));
		m_swapchain->Build(Graphics::GPUSwapchainDesc());
