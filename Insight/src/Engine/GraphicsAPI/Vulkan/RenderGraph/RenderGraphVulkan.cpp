...
namespace Insight::GraphicsAPI::Vulkan
{
	...
	void GPURenderGraphPassVulkan::Init(Graphics::RenderPass& renderPass)
	{
...

		{
			...
			for (auto& subpass : renderPass.GetSubpassDependencies())
			{
				VkSubpassDependency den = {};
				den.srcSubpass = static_cast<u32>(subpass.SourceSubpass);
				den.dstSubpass = static_cast<u32>(subpass.DestinationSubpass);

				den.srcStageMask = ToVulkanPipelineStageFlags(subpass.SourceStageMask);
				den.dstStageMask = ToVulkanPipelineStageFlags(subpass.DestinationStageMask);
				den.srcAccessMask = ToVulkanAccessFlags(subpass.SourceAccessMask);
				den.dstAccessMask = ToVulkanAccessFlags(subpass.DestinationAccessMask);
				den.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
