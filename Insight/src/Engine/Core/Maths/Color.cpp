...
Color::Color(const glm::vec3& rgb, float a)
    : R(rgb.x)
    , G(rgb.y)
    , B(rgb.z)
    , A(a)
...
Color Color::FromHex(const std::string& hexString, bool& isValid)
{
    I32 r, g, b, a = 255;
    isValid = true;

    I32 startIndex = !hexString.empty() && hexString[0] == '#' ? 1 : 0;
    if (hexString.size() == 3 + startIndex)
    {
        r = HexDigit(hexString[startIndex++]);
        g = HexDigit(hexString[startIndex++]);
        b = HexDigit(hexString[startIndex]);

        r = (r << 4) + r;
        g = (g << 4) + g;
        b = (b << 4) + b;
    }
    else if (hexString.size() == 6 + startIndex)
    {
        r = (HexDigit(hexString[startIndex + 0]) << 4) + HexDigit(hexString[startIndex + 1]);
        g = (HexDigit(hexString[startIndex + 2]) << 4) + HexDigit(hexString[startIndex + 3]);
        b = (HexDigit(hexString[startIndex + 4]) << 4) + HexDigit(hexString[startIndex + 5]);
    }
    else if (hexString.size() == 8 + startIndex)
    {
        r = (HexDigit(hexString[startIndex + 0]) << 4) + HexDigit(hexString[startIndex + 1]);
        g = (HexDigit(hexString[startIndex + 2]) << 4) + HexDigit(hexString[startIndex + 3]);
        b = (HexDigit(hexString[startIndex + 4]) << 4) + HexDigit(hexString[startIndex + 5]);
        a = (HexDigit(hexString[startIndex + 6]) << 4) + HexDigit(hexString[startIndex + 7]);
    }
    else
    {
        r = g = b = 0;
...
Color Color::FromHSV(float hue, float saturation, float value, float alpha)
{
    ...
    const float hDiv60Floor = Maths::Floor(hDiv60);
    ...
    const I32 swizzleIndex = (I32)hDiv60Floor % 6;
...
Color Color::FromHSV(const glm::vec3& hsv, float alpha)
{
...
std::string Color::ToHexString() const
{
    ...
    const Byte r = static_cast<Byte>(R * U8_MAX);
    const Byte g = static_cast<Byte>(G * U8_MAX);
    const Byte b = static_cast<Byte>(B * U8_MAX);
...
{
    const float rgbMin = Maths::Min(R, G, B);
    const float rgbMax = Maths::Max(R, G, B);
    const float rgbRange = rgbMax - rgbMin;
    const float hue = rgbMax == rgbMin ? 0.0f : rgbMax == R ? Maths::Mod((G - B) / rgbRange * 60.0f + 360.0f, 360.0f) : rgbMax == G ? (B - R) / rgbRange * 60.0f + 120.0f : rgbMax == B ? (R - G) / rgbRange * 60.0f + 240.0f : 0.0f;
...
{
#define LINEAR_TO_SRGB(value) value < 0.00313067f ? value * 12.92f : Maths::Pow(value, (1.0f / 2.4f)) * 1.055f - 0.055f
...
{
#define SRGB_TO_LINEAR(value) value < 0.04045f ? value / 12.92f : Maths::Pow((value + 0.055f) / 1.055f, 2.4f)
    return Color(SRGB_TO_LINEAR(srgb.R), SRGB_TO_LINEAR(srgb.G), SRGB_TO_LINEAR(srgb.B), SRGB_TO_LINEAR(srgb.A));
#undef LINEAR_TO_SRGB
}

U32 Color::GetHashCode() const
{
    const I32 range = 100000;
    I32 hashCode = (I32)R * range;
    hashCode = hashCode * 397 ^ (I32)(G * range);
    hashCode = hashCode * 397 ^ (I32)(B * range);
    hashCode = hashCode * 397 ^ (I32)(A * range);
    return hashCode;
}
