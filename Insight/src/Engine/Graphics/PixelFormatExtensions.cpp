...
namespace
{
    I32 sizeOfInBits[MAX_PIXEL_FORMATS];

    I32 GetIndex(const PixelFormat format)
    {
        return (I32)format;
    }
}

#define InitFormat(formats, bitCount) for(int i = 0; i < ARRAY_SIZEOF(formats); i++) { sizeOfInBits[GetIndex(formats[i])] = bitCount; }
...
I32 PixelFormatExtensions::SizeInBits(PixelFormat format)
{
    return sizeOfInBits[GetIndex(format)];
}

I32 PixelFormatExtensions::AlphaSizeInBits(const PixelFormat format)
{
    switch (format)
    {
...
int PixelFormatExtensions::ComputeScanlineCount(const PixelFormat format, I32 height)
{
    switch (format)
    {
    case PixelFormat::BC1_Typeless:
