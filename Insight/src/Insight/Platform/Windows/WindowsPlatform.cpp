...
namespace
{
    std::string UserLocale, ComputerName, UserName;
...
I32 CalculateDpi(HMODULE shCoreDll)
{
    I32 dpiX = 96, dpiY = 96;
...
LONG GetStringRegKey(HKEY hKey, const Char* strValueName, std::string& strValue, const std::string& strDefaultValue)
{
...
LONG CALLBACK SehExceptionHandler(EXCEPTION_POINTERS* ep)
{
    ...
    // Get exception info
    std::string errorMsg(TEXT("Unhandled exception: "));
...
#undef CASE
    case EXCEPTION_ACCESS_VIOLATION:
        ...
        errorMsg += std::string::Format(TEXT("{:#x}"), (U32)ep->ExceptionRecord->ExceptionInformation[1]);
        break;
    default:
        errorMsg += std::string::Format(TEXT("{:#x}"), (U32)ep->ExceptionRecord->ExceptionCode);
    }

    // Pause if debugging
    if (Platform::IsDebuggerPresent())
    {
        IS_PLATFORM_DEBUG_BREAK;
    }
...
DialogResult MessageBox::Show(Window* parent, const std::string_view& text, const std::string_view& caption, MessageBoxButtons buttons, MessageBoxIcon icon)
{
...
bool WindowsPlatform::ReadRegValue(void* root, const std::string& key, const std::string& name, std::string* result)
{
    HKEY hKey;
...
void WindowsPlatform::LogInfo()
{
    ...
    // Get OS version
    {
        ...
        if (lRes == ERROR_SUCCESS)
        {
            ...
            if (VersionMajor == 0 && VersionMinor == 0)
            {
                ...
                {
                    const I32 dot = windowsVersion.Find('.');
                    if (dot != -1)
                    {
    ...
    {
        auto msg = std::string::Format(TEXT("Not enough RAM memory for good application performance.\nDetected: {0} MB\nRecommended : {1} MB\nDo you want to continue ?"), mb, mbMinimum);
...
void WindowsPlatform::Log(const std::string_view& msg)
{
    OutputDebugStringW(msg.Get());
    OutputDebugStringW(TEXT(IS_PLATFORM_LINE_TERMINATOR));
}
...
I32 WindowsPlatform::GetDpi()
{
    return SystemDpi;
}

std::string WindowsPlatform::GetUserLocaleName()
{
    return UserLocale;
}

std::string WindowsPlatform::GetComputerName()
{
    return ComputerName;
}
...
bool WindowsPlatform::CanOpenUrl(const std::string_view& url)
{
    return true;
}

void WindowsPlatform::OpenUrl(const std::string_view& url)
{
...
void WindowsPlatform::GetEnvironmentVariables(Dictionary<std::string, std::string>& result)
{
    ...
    if (environmentStr)
    {
        ...
        while (*env != '\0')
        {
            if (*env != '=')
            {
                ...
                ASSERT(str);
                result[std::string(env, (I32)(str - env))] = str + 1;
            }
            while (*env != '\0')
                env++;
            env++;
...
bool WindowsPlatform::GetEnvironmentVariable(const std::string& name, std::string& value)
{
...
I32 WindowsPlatform::StartProcess(const std::string_view& filename, const std::string_view& args, const std::string_view& workingDir, bool hiddenWindow, bool waitForEnd)
{
    // Info
...
I32 WindowsPlatform::RunProcess(const std::string_view& cmdLine, const std::string_view& workingDir, bool hiddenWindow)
{
    return RunProcess(cmdLine, workingDir, Dictionary<std::string, std::string>(), hiddenWindow);
}
...
void ReadPipe(HANDLE pipe, std::vector<char>& rawData, std::vector<Char>& logData, LogType logType)
{
    ...
    if (PeekNamedPipe(pipe, nullptr, 0, nullptr, &bytesAvailable, nullptr) && bytesAvailable > 0)
    {
        ...
        if (ReadFile(pipe, rawData.Get(), bytesAvailable, &bytesRead, nullptr) && bytesRead > 0)
        {
            ...
            logData.Last() = '\0';
            Log::Logger::Write(logType, std::string_view(logData.Get(), rawData.Count()));
        }
    }
}

I32 WindowsPlatform::RunProcess(const std::string_view& cmdLine, const std::string_view& workingDir, const Dictionary<std::string, std::string>& environment, bool hiddenWindow)
{
    ...
    if (captureStdOut)
    {
        ...
        {
            attributeList.Resize((I32)bufferSize);
...
std::vector<PlatformBase::StackFrame> WindowsPlatform::GetStackTrace(I32 skipCount, I32 maxDepth, void* context)
{
    ...
    I32 count;
    void* backtrace[100];
    maxDepth = Math::Min<I32>(maxDepth, ARRAY_COUNT(backtrace));
    if (context)
    {
...
#error "Platform not supported!"
    ...
    for (I32 i = 0; i < count; i++)
    {
        ...
        if (SymGetLineFromAddr64(process, (DWORD64)frame.ProgramCounter, &offset, &line))
        {
            ...
            const I32 fileNameLength = Math::Min<I32>(ARRAY_COUNT(frame.FileName) - 1, StringUtils::Length(line.FileName));
        ...
        {
            const I32 moduleNameLength = Math::Min<I32>(ARRAY_COUNT(frame.ModuleName) - 1, StringUtils::Length(module.ImageName));
...
void WindowsPlatform::CollectCrashData(const std::string& crashDataFolder, void* context)
{
