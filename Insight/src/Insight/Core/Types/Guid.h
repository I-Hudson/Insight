// Copyright (c) 2012-2021 Wojciech Figat. All rights reserved.

#pragma once

#include "Core/Core.h"
#include "Platform/Platform.h"
#include "Core/Templates.h"

/// <summary>
/// Globally Unique Identifier
/// </summary>
struct Guid
{
public:

    /// <summary>
    /// Accepted format specifiers for the format parameter
    /// </summary>
    enum class FormatType
    {
        // 32 digits:
        // 00000000000000000000000000000000
        N,

        // 32 digits separated by hyphens:
        // 00000000-0000-0000-0000-000000000000
        D,

        // 32 digits separated by hyphens, enclosed in braces:
        // {00000000-0000-0000-0000-000000000000}
        B,

        // 32 digits separated by hyphens, enclosed in parentheses:
        // (00000000-0000-0000-0000-000000000000)
        P,
    };

public:

    union
    {
        struct
        {
            // The first component
            U32 A;

            // The second component
            U32 B;

            // The third component
            U32 C;

            // The fourth component
            U32 D;
        };

        // Raw bytes with the GUID
        Byte Raw[16];

        // Raw values with the GUID
        U32 Values[4];
    };

public:

    // Empty Guid (considered as invalid ID)
    static Guid Empty;

public:

    /// <summary>
    /// Empty constructor.
    /// </summary>
    Guid()
    {
    }

    // Creates and initializes a new Guid from the specified components
    // @param a The first component
    // @param b The second component
    // @param c The third component
    // @param d The fourth component
    Guid(U32 a, U32 b, U32 c, U32 d)
        : A(a)
        , B(b)
        , C(c)
        , D(d)
    {
    }

public:

    // Compares two Guids for equality
    // @param left The first Guid to compare
    // @param right The second Guid to compare
    // @returns True if the Guids are equal, otherwise false
    friend bool operator==(const Guid& left, const Guid& right)
    {
        return ((left.A ^ right.A) | (left.B ^ right.B) | (left.C ^ right.C) | (left.D ^ right.D)) == 0;
    }

    // Compares two Guids for inequality
    // @param left The first Guid to compare
    // @param right The second Guid to compare
    // @returns True if the GUIDs are not equal, otherwise false
    friend bool operator!=(const Guid& left, const Guid& right)
    {
        return ((left.A ^ right.A) | (left.B ^ right.B) | (left.C ^ right.C) | (left.D ^ right.D)) != 0;
    }

    // Provides access to the GUIDs components
    // @param index The index of the component to return (0...3)
    // @returns The component value
    U32& operator[](I32 index)
    {
        ASSERT(index >= 0 && index < 4);
        return Values[index];
    }

    // Provides read-only access to the GUIDs components.
    // @param index The index of the component to return (0...3).
    // @return The component
    const U32& operator[](int index) const
    {
        ASSERT(index >= 0 && index < 4);
        return Values[index];
    }

    // Invalidates the Guid
    FORCE_INLINE void Invalidate()
    {
        A = B = C = D = 0;
    }

    // Checks whether this Guid is valid or not. A Guid that has all its components set to zero is considered invalid.
    // @return true if valid, otherwise false
    FORCE_INLINE bool IsValid() const
    {
        return (A | B | C | D) != 0;
    }

    /// <summary>
    /// Checks if Guid is valid
    /// </summary>
    /// <returns>True if Guid isn't empty</returns>
    explicit operator bool() const
    {
        return (A | B | C | D) != 0;
    }

public:

    std::string ToString() const;
    std::string ToString(FormatType format) const;
    void ToString(char* buffer, FormatType format) const;
    void ToString(WChar* buffer, FormatType format) const;

public:

    /// <summary>
    /// Try to parse Guid from string
    /// </summary>
    /// <param name="text">Input text</param>
    /// <param name="value">Result value</param>
    /// <returns>True if cannot parse text, otherwise false</returns>
    static bool Parse(const std::string_view& text, Guid& value);

    /// <summary>
    /// Creates the unique identifier.
    /// </summary>
    /// <returns>New Guid</returns>
    FORCE_INLINE static Guid New()
    {
        Guid result;
        Platform::CreateGuid(result);
        return result;
    }
};

template<>
struct TIsPODType<Guid>
{
    enum { Value = true };
};

inline U32 GetHash(const Guid& key)
{
    return static_cast<I32>(key.A ^ key.B ^ key.C ^ key.D);
}
