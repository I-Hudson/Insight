...
std::string Guid::ToString(FormatType format) const
{
    switch (format)
    {
    case FormatType::N:
        return FormatString("{:0>8x}{:0>8x}{:0>8x}{:0>8x}", A, B, C, D);
    case FormatType::B:
        return FormatString("{{{:0>8x}-{:0>4x}-{:0>4x}-{:0>4x}-{:0>4x}{:0>8x}}}", A, B >> 16, B & 0xFFFF, C >> 16, C & 0xFFFF, D);
    case FormatType::P:
        return FormatString("({:0>8x}-{:0>4x}-{:0>4x}-{:0>4x}-{:0>4x}{:0>8x})", A, B >> 16, B & 0xFFFF, C >> 16, C & 0xFFFF, D);
    default:
        return FormatString("{:0>8x}-{:0>4x}-{:0>4x}-{:0>4x}-{:0>4x}{:0>8x}", A, B >> 16, B & 0xFFFF, C >> 16, C & 0xFFFF, D);
...
bool Guid::Parse(const std::string_view& text, Guid& value)
{
    return GuidParse<std::string, std::string_view>(text, value);
}
