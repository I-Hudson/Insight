...
namespace Platform
{
	...
	Device::Device(const std::vector<const char*> validationLayers, const std::vector<const char*> deviceExtensions, const std::vector<const char*> requiredExtensions)
		...
		, m_deviceExtensions(deviceExtensions)
	{
		VkInstanceCreateInfo createInfo = VulkanInits::InstanceCreateInfo(VulkanInits::ApplicationInfo(CONFIG_VAL(Config::WindowConfig.WindowTitle).c_str()),
			requiredExtensions, validationLayers, &VulkanInits::DebugCreateInfoEXT(DebugCallback));
	...
	{
		ThrowIfFailed(glfwCreateWindowSurface(m_instance, Insight::Module::WindowModule::GetWindow()->m_window, nullptr, &m_surface));
	}
